...
public class DaoVideojuego {
	...
	public boolean comprobar(Videojuego v) {
		Videojuego vDif;
		boolean comprobante = false;
		
		for(int i=0;i<listajuegos.size();i++) {
			vDif = listajuegos.get(i);
			if(v.getId() == vDif.getId() || v.getNombre() == vDif.getNombre()) {
				System.out.println("No se ha añadido el videojuego");
				comprobante = true;
			}
		}
		return comprobante;
